...
In Red the first block is xorred with the plain text to obtain the immediate state in green. When decrypting the immediate state in green is xorred with the $$IV$$ in black obtaining the target plain text in yellow. If we find (using the $$register$$ function to generate long usernames) two blocks (red and block) such that $$P_i = T_i \oplus IV_i \oplus C_i$$ (where $$T$$ is the target plaintext, $$IV$$ and $$C$$ are the two blocks) and $$P_i$$ are all printable characters inside a json string (only printable characters and no " and other escaped characters).
We can obtain the plaintext necessary for obtaining the target plaintext $$T$$ by passing to the $$regiter$$ function the same string to generate $$C$$ and another block $$P$$ obtained before. The result of the encryption contains the ciphtertext in pink of such block that passed to the $$login$$ function as second block, as first block the $$IV$$ found before and as IV the necessary IV to decrypt the first block as what we want. As the plaintext of every block is known we can do that. Obviously we need a correct padding and correct json string as target.

To continue concatenating block we can search for another ciphertext $$C2$$ that satisfy the previous condition: $$P2_i = T2_i \oplus C_i \oplus C2_i$$ and $$P2$$ is printable and json escaped as before. Now we can generate the ciphertext relative to $$P2$$ by generating using the same string inside the $$register$$ and appending $$P2$$ after the plaintext of $$C2$$. After that we have now 3 blocks that we can use to generate whatever string we want inside the json, we can continue and search for a forth block by searching $$C3$$ such that $$P3_i = T2_i \oplus C2_i \oplus C3_i$$ and $$P3$$ is printable and json escaped etc...
